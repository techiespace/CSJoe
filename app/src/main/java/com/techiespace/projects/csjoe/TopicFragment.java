...
public class TopicFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState){
        ...
        response = new FirestoreRecyclerOptions.Builder<Topic>()
                ...
                .build();
        sadapter = new FirestoreRecyclerAdapter<Topic, TopicFragment.ViewHolder>(response) {
            @Override
            public TopicFragment.ViewHolder onCreateViewHolder(ViewGroup group, int viewType) {
                View view = LayoutInflater.from(group.getContext())
                        .inflate(R.layout.list_item, group, false);
                return new TopicFragment.ViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(TopicFragment.ViewHolder holder, int position, Topic model) {
                DocumentSnapshot snapshot = getSnapshots().getSnapshot(holder.getAdapterPosition());
                //progressBar.setVisibility(View.GONE);
                holder.textName.setText(model.getTopic_name());
                //Toast.makeText(getActivity(), "onBindViewHolder: ...."+model.getTopic_name(), Toast.LENGTH_SHORT).show();
